...
interface MetalOverviewItem {
  ...
  exceedCount: number;
  elutionCount: number;
  elutionExceed: number;
}

interface WasteTypePassFail {
  name: string;
  pass: number;
  fail: number;
}
...
const QualityDashboard: React.FC = () => {
  ...
  const metalOverview = useMemo((): MetalOverviewItem[] => {
    return Object.entries(ELUTION_THRESHOLDS).map(([key, threshold]) => {
      ...
      const elutionValues = withElution
        .map(r => {
          const result = r.elutionResult as Record<string, unknown> | null | undefined;
          return result?.[key];
        })
  ...
  const byWasteType = useMemo((): WasteTypePassFail[] => {
    const types: Record<string, WasteTypePassFail> = {};
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={6} sm={3}>
          <Box className="kpi-card">
            ...
            <div className="kpi-card__value" style={{ color: passRate !== '-' && parseFloat(passRate) >= 90 ? PALETTE.success.main : PALETTE.warning.main }}>
              {passRate}
                    ...
                    return (
                      <TableRow key={m.key}>
                        ...
                        <TableCell align="right" sx={{
                          ...
                          color: maxExceeds ? PALETTE.error.main : 'inherit',
            ...
            {byWasteType.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={byWasteType} margin={{ top: 10, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <RechartsTooltip />
                  <Bar dataKey="pass" name="合格" fill={PALETTE.success.main} radius={[4, 4, 0, 0]} {...animProps} />
                  <Bar dataKey="fail" name="不合格" fill={PALETTE.error.main} radius={[4, 4, 0, 0]} {...animProps} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </Box>
            ...
            {recentResults.length === 0 ? (
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table size="small">
                  ...
                  <TableBody>
                    {recentResults.map(r => {
                      const elution = r.elutionResult as Record<string, unknown> | null | undefined;
                      return (
                        <TableRow key={r.id}>
                          ...
                          {Object.entries(ELUTION_THRESHOLDS).slice(0, 4).map(([key, threshold]) => {
                            const val = elution?.[key];
                            const numVal = typeof val === 'number' ? val : undefined;
                            const exceeded = numVal !== undefined && numVal > threshold.limit;
                            return (
                              <TableCell key={key} align="right" sx={{
                                fontFamily: "'Fira Code', monospace",
                                color: exceeded ? PALETTE.error.main : numVal !== undefined ? PALETTE.success.main : 'inherit',
                                fontWeight: exceeded ? 600 : 400
                              }}>
                                {numVal !== undefined ? numVal.toFixed(4) : '-'}
                              </TableCell>
                            );
                          })}
                          <TableCell align="center">
