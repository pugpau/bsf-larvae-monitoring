...
import { CHART_SERIES_COLORS, GRID_STROKE, getCorrelationColor } from '../../constants/colors';
import { useReducedMotion } from '../../hooks/useReducedMotion';
import { getAnimationProps } from '../../utils/recharts';
import { calculateCorrelation, calculateR2 } from '../../utils/statistics';
...
const ANALYSIS_METRICS: MetricDef[] = [
...
const FORMULATION_METRICS: MetricDef[] = [
  { value: 'solidifierAmount', label: '固化剤添加量', unit: 'kg/t' },
  { value: 'suppressorAmount', label: '抑制材添加量', unit: 'kg/t' }
];

const ALL_METRICS: MetricDef[] = [...ANALYSIS_METRICS, ...FORMULATION_METRICS];

const getCorrelationStrength: (val: number) => CorrelationStrength = getCorrelationColor;

const getMetricValue = (record: WasteRecord, metricKey: string): number | null => {
  if (FORMULATION_METRICS.some(m => m.value === metricKey)) {
    const val = record.formulation?.[metricKey];
...
const ScatterTooltip: React.FC<ScatterTooltipProps> = ({ active, payload }) => {
  ...
  return (
    <Box sx={{
      bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1.5,
      ...
      <Typography variant="body2" sx={{ color: 'primary.main' }}>X: {d.x?.toFixed(4)}</Typography>
      <Typography variant="body2" sx={{ color: 'warning.main' }}>Y: {d.y?.toFixed(4)}</Typography>
    </Box>
  );
};

type ViewMode = 'scatter' | 'matrix' | 'effect';
...
const CorrelationAnalysis: React.FC = () => {
  ...
  const [allRecords, setAllRecords] = useState<WasteRecord[]>([]);
  ...
  const scatterData = useMemo((): ScatterDataPoint[] => {
    return filteredRecords
      ...
      .filter((d): d is ScatterDataPoint & { x: number; y: number } =>
        d.x !== null && d.y !== null && !isNaN(d.x) && !isNaN(d.y));
  }, [filteredRecords, xAxis, yAxis]);

  const correlation = useMemo(() => {
    if (scatterData.length < 3) return 0;
    return calculateCorrelation(
      scatterData.map(d => d.x as number),
      scatterData.map(d => d.y as number)
    );
  }, [scatterData]);
  ...
  const correlationMatrix = useMemo(() => {
    ...
    const matrix = metricsToShow.map(m1 => {
      const row: Record<string, string | number | null> = { metric: m1.label, key: m1.value };
      metricsToShow.forEach(m2 => {
        const pairs = filteredRecords
          .map(r => [getMetricValue(r, m1.value), getMetricValue(r, m2.value)] as const)
          .filter((pair): pair is readonly [number, number] => pair[0] !== null && pair[1] !== null && !isNaN(pair[0]) && !isNaN(pair[1]));
        if (pairs.length >= 3) {
  ...
  const effectivenessData = useMemo((): EffectivenessItem[] => {
    return records
      .filter(r => r.elutionResult && r.analysis)
      .map(r => {
        const elution = r.elutionResult as Record<string, unknown> | null;
        const metals = Object.keys(ELUTION_THRESHOLDS);
        const reductions = metals
          .filter(m => r.analysis?.[m] && elution?.[m])
          .map(m => {
            const before = r.analysis![m] as number;
            const after = elution![m] as number;
            return {
  ...
  const handleExport = () => {
    ...
    downloadBlob(blob, `correlation_${new Date().toISOString().split('T')[0]}.csv`);
  };

  const handleSelectChange = (setter: (val: string) => void) =>
    (e: SelectChangeEvent<string>) => setter(e.target.value);
      ...
      <Box className="section-panel" sx={{ mb: 3 }}>
        ...
        <Box className="filter-bar">
          <FormControl size="small" sx={{ minWidth: 140 }}>
            ...
            <Select value={wasteTypeFilter} onChange={handleSelectChange(setWasteTypeFilter)} label="廃棄物種別">
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            ...
            <Select value={xAxis} onChange={handleSelectChange(setXAxis)} label="X軸">
              {ALL_METRICS.map(m => <MenuItem key={m.value} value={m.value}>{m.label}</MenuItem>)}
            </Select>
          </FormControl>
          <FormControl size="small" sx={{ minWidth: 160 }}>
            <InputLabel>Y軸</InputLabel>
            <Select value={yAxis} onChange={handleSelectChange(setYAxis)} label="Y軸">
              {ALL_METRICS.map(m => <MenuItem key={m.value} value={m.value}>{m.label}</MenuItem>)}
            </Select>
          </FormControl>
            ...
            <ResponsiveContainer width="100%" height={400}>
              <ScatterChart margin={{ top: 20, right: 30, bottom: 40, left: 50 }}>
                ...
                {wasteTypeFilter === 'all' && wasteTypes.length > 1 ? (
                  ...
                  <Scatter name="データ" data={scatterData} fill={CHART_SERIES_COLORS[0]} {...animProps} />
                )}
              </ScatterChart>
            </ResponsiveContainer>
          )}
        ...
        <Box className="section-panel" sx={{ mb: 3, overflow: 'auto' }}>
          ...
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, position: 'sticky', left: 0, bgcolor: 'grey.100', zIndex: 1 }}>指標</TableCell>
              ...
              <TableBody>
                {correlationMatrix.matrix.map(row => (
                  <TableRow key={row.key as string}>
                    <TableCell sx={{ fontWeight: 500, fontSize: 12, position: 'sticky', left: 0, bgcolor: 'background.paper', zIndex: 1 }}>
                      {row.metric as string}
                    </TableCell>
                    {correlationMatrix.metrics.map(m => {
                      const val = row[m.value];
                      if (val === null || typeof val !== 'number') return <TableCell key={m.value} align="center">-</TableCell>;
                      ...
                      return (
                        <TableCell key={m.value} align="center" sx={{
                          ...
                          fontSize: 11,
                          bgcolor: isIdentity ? 'grey.100' : undefined,
                          color: isIdentity ? 'text.secondary' : s.color,
          ...
          {effectivenessData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={effectivenessData} layout="vertical" margin={{ top: 10, right: 30, bottom: 10, left: 120 }}>
                ...
                />
                <RechartsTooltip
                  formatter={(val: number) => [`${val.toFixed(1)}%`, '平均低減率']}
                />
                <ReferenceLine x={50} stroke={CHART_SERIES_COLORS[4]} strokeDasharray="3 3" label={{ value: '50%', position: 'top', fontSize: 11 }} />
                <Bar dataKey="avgReduction" radius={[0, 4, 4, 0]} {...animProps}>
